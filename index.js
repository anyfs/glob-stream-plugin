...
var glob = require('anyfs-glob-plugin').glob;
...
var gs = {
  ...
  createStream: function(fs, ourGlob, negatives, opt) {
...
function resolveGlob(fs, glob, opt) {
  var mod = '';
  if (glob[0] === '!') {
    mod = glob[0];
    glob = glob.slice(1);
  ...
  if (opt.root && glob[0] === '/') {
    glob = fs.resolve(opt.root, '.'+glob);
  } else {
    glob = fs.resolve(opt.cwd, glob);
  }
  return mod+glob;
}
